...
class Building:
    ...
    def _addHeatPump(self, data, operationTempertures, temperatureAmb, opt, mergeLinkBuses, mergeHeatSourceSink, dispatchMode, temperatureLevels):
        ...
        from_bus_identifier = _ent.TransformerLabels.from_bus
        to_bus_identifier = _ent.TransformerLabels.to
        if mergeLinkBuses and data[from_bus_identifier] in self.__linkBuses:
            inputBusLabel = [data[from_bus_identifier]]
        elif mergeLinkBuses and any([b in self.__linkBuses for b in data[from_bus_identifier].split(',')]):
            inputBusLabel = [i + '__' + self.__buildingLabel for i in data[from_bus_identifier].split(",") if
                             i not in self.__linkBuses]
            inputBusLabel.extend([i for i in data[from_bus_identifier].split(",") if i in self.__linkBuses])
        elif mergeHeatSourceSink and any([b in self.__heatSourceSinkBuses for b in data[from_bus_identifier].split(',')]):
            inputBusLabel = [i + '__' + self.__buildingLabel for i in data[from_bus_identifier].split(",") if
                             i not in self.__heatSourceSinkBuses]
            inputBusLabel.extend([i for i in data[from_bus_identifier].split(",") if i in self.__heatSourceSinkBuses])
        else:
            inputBusLabel = [i + '__' + self.__buildingLabel for i in data[from_bus_identifier].split(",")]
        inputBuses = [self.__busDict[i] for i in inputBusLabel]
        outputBuses = []
        outputSHBusLabel = data[to_bus_identifier].split(",")[0] + '__' + self.__buildingLabel
        outputDHWBusLabel = data[to_bus_identifier].split(",")[1] + '__' + self.__buildingLabel
        ...
        if temperatureLevels:
            outputBusLabel3 = data[to_bus_identifier].split(",")[2] + '__' + self.__buildingLabel  # outputSHBusLabel, outputDHWBusLabel, outputBusLabel3 are in the order of increasing temperatures
        ...
        if (_ent.HeatPumpCoefficientLabels.coef_W in data) and (_ent.HeatPumpCoefficientLabels.coef_Q in data):
            ...
            coefW = [float(c) for c in data[_ent.HeatPumpCoefficientLabels.coef_W].split(",")]
            coefQ = [float(c) for c in data[_ent.HeatPumpCoefficientLabels.coef_Q].split(",")]
        heatPump = HeatPumpLinear(label.full_name, operationTempertures, temperatureAmb,
                                  coefW,
                                  coefQ,
                                  inputBuses,
                                  outputBuses,
