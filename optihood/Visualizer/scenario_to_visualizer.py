...
from optihood.entities import StorageTypes, IceStorageTypes
...
def get_energy_type_based_on_both_labels(label: str, other_label: str) -> EnergyTypes:
    ...
    storage_parts = [s.value
                     for s_type in (StorageTypes,IceStorageTypes)
                     for s in s_type
                     if s not in [StorageTypes.shStorage, StorageTypes.dhwStorage, StorageTypes.electricalStorage]]
...
class BusesConverter(ScenarioToVisualizerAbstract):
    ...
    def set_from_dataFrame(df: _pd.DataFrame, nr_of_buildings: int) -> _abc.Sequence[_tp.Type[ScenarioToVisualizerAbstract]]:
        ...
        for i, line in df.iterrows():
            ...
            if line['excess']:
                list_of_buses.append(
                    DemandConverter("excess_" + line['label'], None, None,
                                    active=line['active'], building=line['building'],
            ...
            if line['shortage']:
                list_of_buses.append(
                    CommoditySourcesConverter("shortage_" + line['label'], None, None,
                                              active=line['active'], building=line['building'],
        ...
        for i, line in df.iterrows():
            ...
            list_of_demands.append(
                StoragesConverter(line['label'], from_nodes,
                                  to_nodes,
                                  building=line[store.building.value], active=True,
...
class SolarConverter(ScenarioToVisualizerAbstract):
    ...
    def set_from_dataFrame(df: _pd.DataFrame, nr_of_buildings: int):
        ...
        if 'zenith_angle' not in df.columns:
            df['zenith_angle'] = False
    ...
    def get_PVConverter(line, to_nodes):
        return PVConverter(line['label'], None, to_nodes,
    ...
    def get_SolarCollectorConverter(line, from_nodes, to_nodes):
        return SolarCollectorConverter(line['label'], from_nodes, to_nodes,
                                       building=line[solar.building.value], active=True,
...
class LinksConverter(ScenarioToVisualizerAbstract):
    ...
    @staticmethod
    def get_to_and_from_nodes(line, nr_of_buildings: int):
        label = line['label']
    ...
    def set_from_dataFrame(df: _pd.DataFrame, nr_of_buildings: int):
        ...
        for i, line in df.iterrows():
            ...
            list_of_links.append(
                LinksConverter(
                    line['label'], to_nodes, from_nodes,
                    active=line['active'],
