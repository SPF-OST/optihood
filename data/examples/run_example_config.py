...
def run_config_example():
    ...
    network.createScenarioFile(os.path.join(inputFilePath, configFileName), os.path.join(inputFilePath, inputfileName), numberOfBuildings)
    network.setFromExcel(os.path.join(inputFilePath, inputfileName), numberOfBuildings, opt=optimizationType, mergeLinkBuses=mergeLinkBuses, dispatchMode=dispatchMode)
    ...
    limit, capacitiesTransformers, capacitiesStorages = network.optimize(solver='gurobi', numberOfBuildings=numberOfBuildings,
                                                                         options=optimizationOptions, mergeLinkBuses=mergeLinkBuses)
