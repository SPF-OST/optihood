...
def get_current_system_state(system_state: dict[str, float]) -> dict[str, float]:
    ...
    In a real case, the User will need to connect the real/virtual system here.
    """
    system_state['electricalStorage__B001'] = _np.random.random()
    system_state['shStorage__B001'] = _np.random.random()
    return system_state
...
if __name__ == '__main__':
    ...
    # TODO adjust this
    resultFilePath = current_dir / ".." / "results"
    resultFileName = "results_MPC_example"

    # initialize parameters
    numberOfBuildings = 1

    # We will use 4 time steps of 1 hour
    example_time_steps = _pd.date_range("2018-01-01 00:00:00", "2018-01-01 04:00:00", freq=f"{str(time_step_in_minutes)}min")

    # The MpcHandler will take care of many things for us.
    mpc = MpcHandler(prediction_window_in_hours=prediction_window_in_hours, time_step_in_minutes=time_step_in_minutes, nr_of_buildings=1)
    ...
    if not _os.path.exists(resultFilePath):
        _os.makedirs(resultFilePath)
    ...
    for current_time_step in example_time_steps:
        ...
        # TODO: adjust name to time step.
        result_file_path = resultFilePath / f"{resultFileName}_{current_time_step.strftime("%Y_%m_%d__%H_%M_%S")}.xlsx"
